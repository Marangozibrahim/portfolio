...
**Content lives in `src/data/`, never in components.** `profile.ts`, `projects.ts`, `experience.ts`, `skills.ts`, `statement.ts` hold all CV content, typed by interfaces in `src/types.ts`. To update site text/projects/skills, edit data files only — components render whatever the data layer provides. Exception: the terminal hero owns its own scripts in `TerminalHero.tsx` — the auto-typed intro (`entries` array) and the interactive command set, built per-language by `buildCommands(lang)`. The command set pulls live content from `profile`/`skills`/`projects`, so adding a command or section there is the place to extend the terminal. `src/data/sections.ts` exports `buildSections(lang)` — a `name → content` map (built fresh per language from the data layer) that backs the terminal's virtual filesystem — `ls` lists its keys, `cat <key>` prints the value as pretty JSON. Edit that builder to change what those commands show.

**Internationalization (EN/TR) is a custom Context, no i18n lib.** Translatable prose fields are typed `L<T> = Record<Lang, T>` (`Lang = "en" | "tr"`) in `types.ts`; flat fields (name, email, urls, tech stacks, periods, dates, skill labels) stay plain strings — only prose is duplicated. `src/i18n/`: `context.ts` (the React context + `STORAGE_KEY`), `LangContext.tsx` (`LangProvider`, wraps the app in `main.tsx`), `useLang.ts` (`useLang()`, `useT()`, `pick()`). **The 3-file split is deliberate** — `react-refresh/only-export-components` forbids exporting hooks/utils from the same file as a component, so `LangContext.tsx` exports ONLY the provider; everything else imports from `useLang`. Components read prose via `const t = useT(); t(profile.tagline)`. `pick(value, lang)` is the non-hook form (used in `sections.ts`/`TerminalHero` factories). **EN fallback:** `pick` returns `v.en` when the active-language string is empty or array is empty, so half-translated `tr: ""` slots render English, never blank. Initial language: `localStorage 'marangoz-lang'` else `navigator.language` (TR→tr, else en); the provider persists the choice and sets `<html lang>`. UI chrome strings (button labels, aria-labels, "open to work") live in `src/data/ui.ts`, also `L<string>`. **Intentionally English (brand/API, not prose):** nav paths (`/about`), section titles, terminal command verbs (`help`/`ls`/`cat`), skill group labels, tech names.
